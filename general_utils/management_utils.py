...
def log_files_initialization(date: str, algorithms_names: list):
    for algorithm_name in algorithms_names:
        log_file = open(os.path.join('Execution_Outputs', date, 'Logs', '{}.txt'.format(algorithm_name)), 'w')
        log_file.close()


def write_in_log_file(date: str, algorithm_name: str, problem: ExtendedProblem, n: int, elapsed_time: float, hv_rel_diff: float):
    log_file = open(os.path.join('Execution_Outputs', date, 'Logs', '{}.txt'.format(algorithm_name)), 'a')
    log_file.write('Problem: ' + problem.__name__ + '    N: ' + str(n) + '    Time: ' + str(elapsed_time) + '    HV Rel Diff: ' + str(hv_rel_diff) + '\n')
    log_file.close()
