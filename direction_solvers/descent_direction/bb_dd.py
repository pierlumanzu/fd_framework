...
class BB_DD(DDS, GurobiSettings):

    def __init__(self, gurobi_method: int, gurobi_verbose: bool):
        DDS.__init__(self)
        GurobiSettings.__init__(self, gurobi_method, gurobi_verbose)

    def compute_direction(self, problem: ExtendedProblem, Jac: np.array, x_p: np.array = None, alpha: np.array = None):
        ...
        if len(Jac) <= 2:
            ...
            try:
                model = Model("Barzilai-Borwein Descent Direction")
                ...
                for j in range(m):
                    model.addConstr(Jac[j, :] @ (z - x_p) / alpha[j] <= beta, name='Jacobian constraint n°{}'.format(j))
