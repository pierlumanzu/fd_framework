...
if general_settings['general_export']:
    ...
    log_files_initialization(date, algorithms_names)
    ...
    for n in var_range:
        ...
        for algorithm_name in algorithms_names:
            ...
            with session.as_default():
                ...
                problem_instance.evaluate_functions_hessians(initial_p_list[0, :])

                p_list, f_list, elapsed_time, hv_rel_diff = algorithm.search(np.copy(initial_p_list), np.copy(initial_f_list), problem_instance)
                ...
                if general_settings['general_export']:
                    write_in_log_file(date, algorithm_name, problem, n, elapsed_time, hv_rel_diff)
                    write_results_in_csv_file(p_list, f_list, date, algorithm_name, problem, general_settings['export_pareto_solutions'])
