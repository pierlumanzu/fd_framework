...
class ExtendedProblem(Problem, ABC):
    ...
    def evaluate_functions_hessians(self, x: np.array):

        hessians = np.zeros((self.m, self.n, self.n))
        for i in range(self.m):
            if self.__objectives_hessians[i] is not None:
                hessians[i, :, :] = self.__objectives_hessians[i].eval({self._z: x})

        return hessians
    
    def evaluate_functions_hessian(self, x: np.array, index: int):
        return self.__objectives_hessians[index].eval({self._z: x})
